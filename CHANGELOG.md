...
Two pieces of player feedback: an accessibility request (pop-up text too small to read), and a detailed late-game balance report — FUSION dominating schematic earns, near-full patent mastery after only 2 publishes, and challenges/exhibitions "solving themselves within seconds" once the multiplier stack is tall enough. This patch addresses all of it.

### Added

- **FONT SIZE slider** (Settings → Accessibility, 100%–150%). Scales every piece of text in the game — modals, tooltips, toasts, settings, the factory grid — not just body copy. Implementation: all 186 `font-size` declarations in the stylesheet are now `calc(Npx * var(--fs, 1))` and the slider drives `--fs` on the document root, so popups that previously bottomed out at 7–9 px scale up with everything else. Live preview while dragging (the settings modal itself scales); persists in the save like every other setting. New `font-size` rules must use the same `calc()` form or they won't respond to the slider.

### Balance

- **FUSION blueprint reined in.** Was: +0.02 cores/s per ingot machine, *multiplied by the global production multiplier*. Cores carry the top score weight (6.25M), so at a late-game 20–50× production stack the drip out-earned the entire real T3–T5 chain and became the only blueprint worth picking — the fast-schematic engine behind the "trivial after first publish" report. Now: +0.01 cores/s, flat (power/efficiency state still applies, the production multiplier does not). It remains a strong early/mid-cycle shortcut — cores long before your first forge — and fades once the real chain is industrial, which is what a "shortcut the chain" run modifier should do.
- **Patent softcap tightened** (exponent above 100 raw patents: 0.65 → 0.5). The reporting player banked ~450 patents per publish at FUSION-era prototype counts and owned effectively all of mastery (~904 patents) after 2 publishes — the meta-layer designed for 8–12. The same extreme run now pays ~190; normal-play publishes (≤100K prototypes ≈ 92 patents) are completely unchanged. Combined with the FUSION fix, the prototype counts that fed the old curve are also far harder to reach.
- **Challenge goals now scale with demonstrated power, not just patents.** The v0.9.7 patent curve tops out around 7–8× while the multiplicative bonus stack (achievements × patents × challenge rewards × legacies) pushes real earning power to 30–80× — hence goals completing "automatically within seconds". Goals are now the *higher* of two scales:
  - the patent floor, slightly raised: `1 + sqrt(totalPatents) * 0.25` (was 0.2), and
  - a power anchor: 30% of your average schematic earn per prestige *this publish cycle* (`totalSchematics / prestigeCount`, both of which reset at publish — so goals relax after the big wipe and tighten as the cycle's power rebuilds). BLITZ uses 12% (a 5-minute window can't match a full run) and BLACKOUT uses 15% (banning research cuts out most of late-cycle power).
  - The goal is **frozen when you start the challenge** — the number on the card when you commit is the number you're judged on, even if your average moves mid-run. Mastery cards, the start modal, and the active-challenge banner all show the scaled goal as before.
- **Exhibition goals scale too.** The three numeric exhibitions had fixed targets a late-game factory rolled past by accident, which made most pool picks auto-wins:
  - **SCALE SHOW**: max(10M, 60% of your best single-run ore this publish cycle)
  - **EXHIBITION MATCH**: max(5K, 75% of your best single-run schematic earn this cycle)
  - **HEAVY ARTILLERY**: `10 + floor(sqrt(lifetimePatents) / 2)` of every slot-1 machine (25 each at 900 patents)

  Targets freeze when you select the exhibition (the card shows the live number), and per-cycle bests reset at publish so post-publish picks fall back to the old floors while you rebuild. Behavioural exhibitions (SILENT, EMPTY STAGE, MINIATURE WORKS, THE LONG HAUL, GRAND TOUR) are constraint goals, not number goals, and stay as they were.

### Notes

- Save migration is automatic and additive: `settings.fontScale` (default 1), `meta.bestRunOre` / `meta.bestRunSchematics` (default 0), `meta.challenge.goal` (default 0), `meta.exhibitions.activeTargets` (default empty). Nothing existing is rewritten.
- A challenge or exhibition that is *already active* when the update lands keeps working: with no frozen goal stored, the goal is computed live from the new formulas at evaluation (and shown the same way in the UI). If a Publish happens while one is active, its goal re-freezes against the fresh cycle instead of carrying a stale pre-publish number.
- Completed challenges/exhibitions stay completed — nothing has to be re-won at the new scaling.
- Achievement pacing: no new achievements this patch, but the 2-publish mastery rush this report described is the main reason "nearly every achievement in 66 prestiges" was possible. With patents back on the 8–12 publish arc and challenges/exhibitions priced against real power, PROLIFIC (50 publishes), PATENT EMPIRE (1,000 patents), and ARCHIVIST (25 Legacy Marks) gate the way they were designed to in v0.9.9.
